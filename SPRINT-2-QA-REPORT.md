...
Spot-checked manually: GitHub repo URL (`github.com/darco81/cdat-pattern`) resolves; portfolio + LinkedIn external links present in footer.
...
Pattern repo HOLY GROUND maintained throughout 30-min QA pass. Zero `cd ~/dev/darco81/cdat-pattern && git anything` invocations. All pattern repo reads via `curl raw.githubusercontent.com` only.
...
4. **Deploy SDE-52** per `~/dev/darco81/cdat-website/docs/DEPLOY.md` (Sprint 1 deliverable, 8-step playbook).
5. **Update Linear:**
