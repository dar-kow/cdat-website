...
`github.com/darco81/cdat-pattern` HEAD `7dad025`. **DO NOT TOUCH.**
...
```bash
node ~/dev/darco81/sdet-wcag-toolkit/packages/cli/dist/bin/wcag-toolkit.js audit . \
  --multi-page \
...
```bash
cd ~/dev/darco81/sdet-wcag-pro
node packages/cli-pro/dist/bin/wcag-toolkit-pro.js audit ~/dev/darco81/cdat-website \
  --multi-page \
  --strategy=json-config \
  --config ~/dev/darco81/cdat-website/wcag.config.json \
  --output ~/dev/darco81/cdat-website/wcag-rerun-pro.md
```
...
The original script in `~/dev/darco81/sdet-brand-drafts/post-qa-wcag-audit.sh`
...
2. **Replace `pnpm wcag-toolkit` invocations** with direct binary path:
   `node ~/dev/darco81/sdet-wcag-toolkit/packages/cli/dist/bin/wcag-toolkit.js`
...
```bash
cd ~/dev/darco81/sdet-brand-drafts
git pull
...
Write summary to `~/dev/darco81/cdat-website/POST-WCAG-AUDIT-FIXES.md`:
